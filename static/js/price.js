let directions=[],services=[];async function fetchDirections(){try{(directions=await (await fetch(`${window.origin}/services/api/specialities/`)).json()).unshift("Все направления"),renderDirections(directions)}catch(e){console.error("Ошибка при загрузке данных:",e)}}async function fetchServices(){try{services=await (await fetch(`${window.origin}/prices/api/prices/`)).json(),renderServices(services)}catch(e){console.error("Ошибка при загрузке данных:",e)}}function renderDirections(){let e=document.querySelector(".directions");e.innerHTML="",directions.forEach((i,r)=>{let t=`
